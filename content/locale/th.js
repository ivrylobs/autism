export const th = {
    "/": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        about1:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่นได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัย ขอนแก่นในการดูแลบุคคลออทิซึม",
        team: "ทำเนียบบุคลากร",
        team1: "เพื่อให้บริการแก่บุคคลออทิซึมให้ได้รับบริการอย่างทั่วถึงและครอบคลุมในทุกด้าน",
        team2: "ที่ปรึกษา",
        team3: "รักษาการแทนผู้อำนวยการ",
        related: "หน่วยงานที่เกี่ยวข้อง",
        related1: "ฝ่ายวิจัยและการถ่ายทอดเทคโนโลยี คณะแพทยศาสตร์มหาวิทยาลัยขอนแก่น และ...",
        menuWork: "งานของเรา",
        videoWork: "คิดได้คิดดี-แอปพลิเคชัน คัดกรองภาวะออทิซึมสเปกตรัมในเด็ก",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวสารและประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        video: "วีดิทัศน์",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        researchwork1:
            "การพัฒนานวัตกรรมสำหรับการช่วยเหลือบุคคลที่มีภาวะออทิซึมสเปกตรัมที่สอดคล้องกับบริบททางสังคมไทยตาม4 กลไกหลักของออทิซึมสเปกตรัมโรดแมป มีวัตถุประสงค์ตั้งแต่การคิดค้นหาวิธีการคัดกรองเด็กเล็กที่มีอาการที่เสี่ยงต่อการเป็นโรคออทิซึมสเปกตรัมจนถึงค้นหาวิธีการดูแลผู้ป่วยที่เป็นโรคออทิซึมสเปกตรัมตั้งแต่อายุน้อยจนถึงวัยรุ่น ผู้ป่วยที่เป็นเด็กอายุน้อยแผนการวิจัยก็คิดค้นวิธีการให้การศึกษาเพื่อให้การศึกษาก้าวหน้าผู้ป่วยที่เป็นวัยรุ่นแผนการวิจัยก็คิดค้นหาวิธีส่งเสริมให้ครอบครัวมีวิธีการส่งเสริมให้วัยรุ่นมีทักษะทางสังคมดีขึ้นแผนการวิจัยจึงประกอบด้วยโครงการวิจัยย่อย 4 โครงการดังนี้",
    },
    "/home": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
    },
    "/about": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        team: "ทำเนียบบุคลากร",
        related: "หน่วยงานที่เกี่ยวข้อง",
        menuWork: "งานของเรา",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        about1:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรีพลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่นในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม  และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564การสร้างความเป็นธรรมและลดความเหลื่อมล้ำในสังคม  อีกทั้งนโยบายของรัฐบาลในการลดความเหลื่อมล้ำของสังคมและการสร้างโอกาสการเข้าถึงบริการของรัฐและการยกระดับคุณภาพบริการด้านสาธารณสุขและสุขภาพของประชาชน",
        about2:
            "เนื่องจากประเทศไทยมีกลุ่มบุคคลที่เป็นโรคความผิดปกติแต่กำเนิดของพัฒนาการทางสมอง หรือโรคออทิซึมสเปกตรัม (Autistic Spectrum Disorder) ซึ่งพบบ่อยในเด็กส่งผลต่อความบกพร่องต่อความบกพร่องทางสมอง 3 ด้าน คือ ด้านสังคม ภาษา และพฤติกรรม ร่วมกับความผิดปกติอื่น ๆเช่น มีปฏิสัมพันธ์ทางสังคม ไม่สบตา ไม่สนใจคู่สนทนา อ่านใจคนอื่นไม่ออก อยู่ในโลกของตนเอง ทำอะไรซ้ำ ๆ เป็นต้น  ความรุนแรงจะมีความแตกต่างกันขึ้นอยู่กับพัฒนาการทางภาษา ระดับสติปัญญา และความบกพร่องต่าง ๆ จะมีความต่อเนื่องตลอดชีวิตการวินิจฉัยและการรักษาตั้งแต่ในวัยเด็กจะทำให้เด็กออทิซึมสเปกตรัม  ได้มีโอกาสพัฒนาได้ดีกว่าการรักษาเมื่ออายุมากขึ้น การบำบัดรักษาโรคออทิซึมสเปกตรัมต้องดำเนินการอย่างถูกต้องเหมาะสมและเข้มข้น โดยการผสมผสานวิธีการต่าง ๆ ทั้งทางการแพทย์การส่งเสริมพัฒนาการ การจัดรูปแบบการศึกษาที่เหมาะสม การส่งเสริมอาชีพและการมีงานทำ มีสวัสดิการสังคมและการดำเนินชีวิตในชุมชน ดังนั้นการบำบัดจึงต้องมีบุคลากรจากสหวิชาชีพ",
        vision: "วิสัยทัศน์",
        vision1:
            "“พัฒนาคุณภาพชีวิตและสุขภาพของบุคคลออทิซึมสเปกตรัมด้วยงานวิจัยด้านการแพทย์ การศึกษา การสังคม และการกฎหมาย”",
        about3:
            " แต่ที่ผ่านมาพบปัญหาสำคัญคือ ประชาชนยังมีความเข้าใจเรื่องโรคออทิซึมสเปกตรัมน้อยเป็นผลทำให้จำนวนผู้เข้าถึงบริการทางการแพทย์ การศึกษา  และการช่วยเหลือทางสังคมมีน้อย โดยพบเพียงร้อยละ 15 ของผู้ป่วยออทิซึม หรือตามรายงานการสำรวจจำนวนผู้ป่วยทางสุขภาพจิตของประเทศตามเขตสาธารณสุขรายจังหวัด ในปี 2555 มีผู้ป่วยออทิซึมที่เข้าถึงบริการเพียง 25,537 ราย",
        join: "ร่วมเป็นส่วนหนึ่งกับเรา",
        join1: "พัฒนางานวิจัยและบริการที่มีคุณภาพแก่กลุ่มบุคคลออทิซึม และบุคคลต่าง ๆ ที่เกี่ยวข้อง",
        mission: "พันธกิจ",
        mission1:
            "“สถาบันวิจัยและบริการด้านออทิซึมจะปรับปรุงสุขภาพ  และคุณภาพชีวิตของบุคคลออทิซึมสเปกตรัม ผ่านกระบวนการวิจัยกระบวนการวิชาการ กระบวนการให้บริการ และกระบวนการทางกฎหมายโดยผู้เชี่ยวชาญและผู้มีส่วนเกี่ยวข้องอื่น ๆ รวมทั้งครอบครัวของบุคคลที่มีภาวะออทิซึมสเปกตรัม”",
        about4:
            "ดังนั้น จากเหตุผลและความจำเป็นดังกล่าว มหาวิทยาลัยขอนแก่น จึงได้จัดตั้ง “สถาบันวิจัยและบริการด้านออทิซึม”สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ตั้งแต่วันที่ 7 มิถุนายน2561 เพื่อรองรับการบริการบุคคลออทิซึมให้มีมาตรฐานและมีการพัฒนาบุคคลออทิซึมในทุกระดับ ทุกเพศทุกวัยรวมไปถึงผู้ปกครอง ครู แพทย์ พยาบาล และบุคคลที่เกี่ยวข้องต่าง ๆพร้อมกับการดำเนินการวิจัยร่วมกับการบริการที่มีคุณภาพ",
        objectives: "วัตถุประสงค์การจัดตั้ง",
        objectives1:
            "1) เพื่อให้บริการแก่บุคคลออทิซึมให้ได้รับการบริการอย่างทั่วถึงและครอบคลุมในทุกด้านให้สามารถอยู่ร่วมกับสังคมได้อย่างมีความสุข ",
        objectives2:
            "2)เพื่อพัฒนาศักยภาพบุคลากรทางการแพทย์และที่เกี่ยวข้องในการดูแลบุคคลออทิซึมได้ตามความต้องการของประเทศ",
        objectives3:
            "3) เพื่อวิจัยค้นหาสาเหตุของโรคออทิซึม วิธีป้องกันวิธีรักษาให้สามารถนำมาเป็นองค์ความรู้เพื่อการป้องกันการรักษาโรคออทิซึม",
        objectives4:
            "4) เพื่อพัฒนาเครื่องมือคัดกรอง การวินิจฉัยคู่มือการดูแลบุคคลออทิซึมที่ได้มาตรฐานสามารถนำไปใช้ได้อย่างแพร่หลายได้อย่างสะดวกสบาย",
        objectives5: "5)  เพื่อส่งเสริมเครือข่ายความร่วมมือเพื่อให้บริการทางการแพทย์และการรักษาพยาบาลแก่บุคคลออทิซึม",
    },
    "/team": {
        hello: "Hello, world!",
        welcomeMessage: "Welcome to your internationalised page!",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        team: "ทำเนียบบุคลากร",
        related: "หน่วยงานที่เกี่ยวข้อง",
        menuWork: "งานของเรา",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        member1: "ที่ปรึกษา",
        member11: "รักษาการแทนผู้อำนวยการ",
        member111: "กรรมการและเลขานุการ",
        member1111: "ผู้ช่วยเลขานุการ",
        member111111: "กรรมการ",
        member1111111: "งานวิชาการและบริการ",
        member11111111: "งานการเงินและงบประมาณ",
        member111111111: "งานพัสดุและธุรการ",
    },
    "/related": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        team: "ทำเนียบบุคลากร",
        related: "หน่วยงานที่เกี่ยวข้อง",
        menuWork: "งานของเรา",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        related: "หน่วยงานที่เกี่ยวข้อง",
        related1: "• ฝ่ายวิจัยและการถ่ายทอดเทคโนโลยี",
        related2: "• คณะแพทยศาสตร์ มหาวิทยาลัยขอนแก่น",
        related3: "• คณะศึกษาศาสตร์ มหาวิทยาลัยขอนแก่น",
        related4: "• คณะพยาบาลศาสตร์ มหาวิทยาลัยขอนแก่น",
        related5: "• คณะศิลปกรรมศาสตร์ มหาวิทยาลัยขอนแก่น",
        related6: "• โรงเรียนสาธิตมอดินแดง มหาวิทยาลัยขอนแก่น",
        related7: "• โรงเรียนสาธิตศึกษาศาสตร์ มหาวิทยาลัยขอนแก่น",
        related8: "• โรงพยาบาลศรีนครินทร์",
        related9: "• โรงพยาบาลขอนแก่น",
        related10: "• สมาคมผู้ปกครองออทิซึม",
        related11: "• มูลนิธิขอบฟ้ากว้าง",
        related12: "• ศูนย์พัฒนาศักยภาพบุคคลออทิสติกจังหวัดขอนแก่น",
        related13: "• ศูนย์การศึกษาพิเศษ เขตการศึกษา 9 จังหวัดขอนแก่น",
        related14: "• สถาบันสุขภาพจิตเด็กและวัยรุ่นภาคตะวันออกเฉียงเหนือ",
    },
    "/works/research": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        team: "ทำเนียบบุคลากร",
        related: "หน่วยงานที่เกี่ยวข้อง",
        menuWork: "งานของเรา",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        researchwork: "งานด้านการวิจัย",
        researchwork1:
            "การพัฒนานวัตกรรมสำหรับการช่วยเหลือบุคคลที่มีภาวะออทิซึมสเปกตรัมที่สอดคล้องกับบริบททางสังคมไทยตาม4 กลไกหลักของออทิซึมสเปกตรัมโรดแมป มีวัตถุประสงค์ตั้งแต่การคิดค้นหาวิธีการคัดกรองเด็กเล็กที่มีอาการที่เสี่ยงต่อการเป็นโรคออทิซึมสเปกตรัมจนถึงค้นหาวิธีการดูแลผู้ป่วยที่เป็นโรคออทิซึมสเปกตรัมตั้งแต่อายุน้อยจนถึงวัยรุ่น ผู้ป่วยที่เป็นเด็กอายุน้อยแผนการวิจัยก็คิดค้นวิธีการให้การศึกษาเพื่อให้การศึกษาก้าวหน้าผู้ป่วยที่เป็นวัยรุ่นแผนการวิจัยก็คิดค้นหาวิธีส่งเสริมให้ครอบครัวมีวิธีการส่งเสริมให้วัยรุ่นมีทักษะทางสังคมดีขึ้นแผนการวิจัยจึงประกอบด้วยโครงการวิจัยย่อย 4 โครงการดังนี้",
        subresearch1: "โครงการวิจัยย่อยที่ 1",
        subresearch2: "โครงการวิจัยย่อยที่ 2",
        subresearch3: "โครงการวิจัยย่อยที่ 3",
        subresearch4: "โครงการวิจัยย่อยที่ 4",
        projectreseach1:
            "เป็นการวิจัยพัฒนาสร้างแอพพลิเคชั่นบนทุกระบบปฏิบัติการอินเตอร์เน็ตเพื่อให้พ่อแม่ผู้ปกครองได้ใช้คัดกรองตรวจสอบว่าบุตรหลานมีอาการและมีความเสี่ยงต่อการเป็นโรคออทิซึมสเปกตรัมหรือไม่(ผู้ประดิษฐ์: ผศ.พญ.กุศลาภรณ์ ชัยอุดมสม)",
        projectreseach2:
            "เป็นการวิจัยพัฒนาสร้างโปรแกรมให้ครูในโรงเรียนเรียนรวมนำไปใช้สอนเด็กประถมศึกษาที่มีโรคออทิซึมสเปกตรัมเพื่อเพิ่มทักษะทางด้านวิชาการและด้านสังคม(ผู้ประดิษฐ์: รศ.ดร.ปิยะวรรณ ศรีสุรักษ์)",
        projectreseach3:
            "เป็นการวิจัยพัฒนาสร้างโปรแกรมให้ครูในโรงเรียนเรียนรวมนำไปใช้สอนเด็กอนุบาลที่มี โรคออทิซึมสเปกตรัมเพื่อเพิ่มทักษะทางด้านวิชาการและด้านภาษา (ผู้ประดิษฐ์: นางพรมณี หาญหัก)",
        projectreseach4:
            "เป็นการวิจัยพัฒนาสร้างโปรแกรมให้พ่อแม่หรือผู้ปกครองนำไปใช้เพื่อเพิ่มทักษะการสังคมให้แก่วัยรุ่นในบ้านที่เป็นโรคออทิซึมสเปกตรัม(ผู้ประดิษฐ์: ดร.ธิรากร มณีรัตน์)",
    },
    "/works/research/active": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        team: "ทำเนียบบุคลากร",
        related: "หน่วยงานที่เกี่ยวข้อง",
        menuWork: "งานของเรา",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        researchactive: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        researchactive1:
            "1. การพัฒนาโปรแกรมการฝึกกิจวัตรประจำวันร่วมกับการใช้ห้องตัวอย่าง สำหรับนักเรียนระดับอนุบาลที่มีภาวะ ออทิซึมสเปกตรัม และครอบครัว (HCP) ",
        researchactive11: "(อ.ดร.กรวรรณ โหม่งพุฒ)",
        researchactive2: " 2. การพัฒนาหลักสูตรการเรียนรวม 5 ทักษะพื้นฐานในผู้เรียนระดับอนุบาลที่มีภาวะออทิซึมสเปกตรัม",
        researchactive22: "(ผศ.ดร.ปิยะวรรณ ศรีสุรักษ์)",
        researchactive222:
            " 2.1 หลักสูตรการเรียนรวมเพื่อพัฒนาทักษะด้านความรู้ความเข้าใจสำหรับนักเรียนที่มีภาวะออทิซึมสเปกตรัมในระดับอนุบาลศึกษา (ผศ.ดร.ปิยะวรรณ ศรีสุรักษ์)",
        researchactive2222:
            " 2.2หลักสูตรการพัฒนาทักษะการรับรู้และแสดงออกทางภาษาสำหรับเด็กอนุบาลที่มีภาวะออทิซึมสเปกตรัมในชั้นเรียนรวม (อ.อาพร ตรีสูน)",
        researchactive22222:
            " 2.3 หลักสูตรเพิ่มสมรรถภาพทางกายสำหรับผู้เรียนระดับอนุบาลศึกษาที่มีภาวะออทิซึมสเปกตรัมในโรงเรียนเรียนร่วม(อ.วันชัย จันทการกุล)",
        researchactive222222:
            "2.4  หลักสูตรการเรียนรวมเพื่อพัฒนาทักษะการช่วยเหลือตนเองในการปฏิบัติกิจวัตรประจำวันในนักเรียนระดับอนุบาลศึกษา ที่มีภาวะออทิซึมสเปกตรัม (อ.เจริญขวัญศรีพันธ์ชาติ)",
        researchactive2222222:
            "2.5หลักสูตรเรียนรวมพัฒนาทักษะทางสังคมของนักเรียนอนุบาลศึกษาที่มีภาวะออทิซึมสเปกตรัม(อ.พรมณี หาญหัก)",
        researchactive3: "3. โปรแกรมการฝึกทักษะงานบ้านสำหรับบุคคลออทิสติกกลุ่มวัยรุ่นที่ไม่มีภาษาพูด",
        researchactive33: "(อาจารย์ปริศนา อานจำปา)",
        researchactive4: "4. การพัฒนาโปรแกรมการฝึกทักษะสังคมสำหรับวัยรุ่นที่มีภาวะออทิซึม",
        researchactive44: "(อาจารย์ปริศนา อานจำปา)",
        researchactive5:
            "5. การพัฒนาหลักสูตรการวางแผนและการจัดระเบียบดำเนินการเพื่อการประกอบอาชีพ ในวัยรุ่นออทิซึมสเปกตรัม",
        researchactive55: "(ดร.เบญจมาภรณ์ ช้อยเครือ)",
        researchactive6:
            "6. การเปลี่ยนแปลงเมทาบอลิซึมของกลูโคสในสมองซึ่งประเมินด้วยเอฟดีจี เพทซีที ในผู้ป่วยออทิซึมหลังการกระตุ้นด้วยไฟฟ้ากระแสตรงผ่านกะโหลกศีรษะ",
        researchactive66: "(รศ.พญ.ภารดี เอื้อวิชญาแพทย์)",
        researchactive7: "7. การดูแลช่วยเหลือแก่ครอบครัวที่มีบุตรออทิสติก",
        researchactive77: "(ผศ. พัชราภรณ์ เจนใจวิทย์)",
    },
    "/works/service": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        team: "ทำเนียบบุคลากร",
        related: "หน่วยงานที่เกี่ยวข้อง",
        menuWork: "งานของเรา",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        service: " งานด้านการบริการ",
        service1: "1. คลินิกให้คำปรึกษาสำหรับบุคคลที่มีภาวะออทิซึมสเปกตรัมโดยผู้เชี่ยวชาญ",
        service2:
            "2.การให้ความช่วยเหลือบุคคลที่มีภาวะออทิซึมสเปกตรัมในชั้นเรียนรวม โรงเรียนสาธิตมหาวิทยาลัยขอนแก่น มอดินแดง ฝ่ายการศึกษาพิเศษ",
    },
    "/post/[id]": {
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        about1:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่นได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัย ขอนแก่นในการดูแลบุคคลออทิซึม",
        team: "ทำเนียบบุคลากร",
        team1: "เพื่อให้บริการแก่บุคคลออทิซึมให้ได้รับบริการอย่างทั่วถึงและครอบคลุมในทุกด้าน",
        team2: "ที่ปรึกษา",
        team3: "รักษาการแทนผู้อำนวยการ",
        related: "หน่วยงานที่เกี่ยวข้อง",
        related1: "ฝ่ายวิจัยและการถ่ายทอดเทคโนโลยี คณะแพทยศาสตร์มหาวิทยาลัยขอนแก่น และ...",
        menuWork: "งานของเรา",
        videoWork: "คิดได้คิดดี-แอปพลิเคชัน คัดกรองภาวะออทิซึมสเปกตรัมในเด็ก",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวสารและประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        researchwork1:
            "การพัฒนานวัตกรรมสำหรับการช่วยเหลือบุคคลที่มีภาวะออทิซึมสเปกตรัมที่สอดคล้องกับบริบททางสังคมไทยตาม4 กลไกหลักของออทิซึมสเปกตรัมโรดแมป มีวัตถุประสงค์ตั้งแต่การคิดค้นหาวิธีการคัดกรองเด็กเล็กที่มีอาการที่เสี่ยงต่อการเป็นโรคออทิซึมสเปกตรัมจนถึงค้นหาวิธีการดูแลผู้ป่วยที่เป็นโรคออทิซึมสเปกตรัมตั้งแต่อายุน้อยจนถึงวัยรุ่น ผู้ป่วยที่เป็นเด็กอายุน้อยแผนการวิจัยก็คิดค้นวิธีการให้การศึกษาเพื่อให้การศึกษาก้าวหน้าผู้ป่วยที่เป็นวัยรุ่นแผนการวิจัยก็คิดค้นหาวิธีส่งเสริมให้ครอบครัวมีวิธีการส่งเสริมให้วัยรุ่นมีทักษะทางสังคมดีขึ้นแผนการวิจัยจึงประกอบด้วยโครงการวิจัยย่อย 4 โครงการดังนี้",
    },
    "/blog": {
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        about1:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่นได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัย ขอนแก่นในการดูแลบุคคลออทิซึม",
        team: "ทำเนียบบุคลากร",
        team1: "เพื่อให้บริการแก่บุคคลออทิซึมให้ได้รับบริการอย่างทั่วถึงและครอบคลุมในทุกด้าน",
        team2: "ที่ปรึกษา",
        team3: "รักษาการแทนผู้อำนวยการ",
        related: "หน่วยงานที่เกี่ยวข้อง",
        related1: "ฝ่ายวิจัยและการถ่ายทอดเทคโนโลยี คณะแพทยศาสตร์มหาวิทยาลัยขอนแก่น และ...",
        menuWork: "งานของเรา",
        videoWork: "คิดได้คิดดี-แอปพลิเคชัน คัดกรองภาวะออทิซึมสเปกตรัมในเด็ก",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวสารและประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        researchwork1:
            "การพัฒนานวัตกรรมสำหรับการช่วยเหลือบุคคลที่มีภาวะออทิซึมสเปกตรัมที่สอดคล้องกับบริบททางสังคมไทยตาม4 กลไกหลักของออทิซึมสเปกตรัมโรดแมป มีวัตถุประสงค์ตั้งแต่การคิดค้นหาวิธีการคัดกรองเด็กเล็กที่มีอาการที่เสี่ยงต่อการเป็นโรคออทิซึมสเปกตรัมจนถึงค้นหาวิธีการดูแลผู้ป่วยที่เป็นโรคออทิซึมสเปกตรัมตั้งแต่อายุน้อยจนถึงวัยรุ่น ผู้ป่วยที่เป็นเด็กอายุน้อยแผนการวิจัยก็คิดค้นวิธีการให้การศึกษาเพื่อให้การศึกษาก้าวหน้าผู้ป่วยที่เป็นวัยรุ่นแผนการวิจัยก็คิดค้นหาวิธีส่งเสริมให้ครอบครัวมีวิธีการส่งเสริมให้วัยรุ่นมีทักษะทางสังคมดีขึ้นแผนการวิจัยจึงประกอบด้วยโครงการวิจัยย่อย 4 โครงการดังนี้",
        find: "ค้นหาหัวข้อเรื่อง",
        found: "ค้นหา",
        nonfind: "ไม่พบข้อมูล",
    },
    "/donate": {
        hello: "สวัสดีครับ",
        welcomeMessage: "ยินดีต้องรับสู่หน้าภาษาไทย",
        menuIndex: "หน้าแรก",
        menuAbout: "เกี่ยวกับเรา",
        about: "ความเป็นมาและวัตถุประสงค์",
        about1:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่นได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัย ขอนแก่นในการดูแลบุคคลออทิซึม",
        team: "ทำเนียบบุคลากร",
        team1: "เพื่อให้บริการแก่บุคคลออทิซึมให้ได้รับบริการอย่างทั่วถึงและครอบคลุมในทุกด้าน",
        team2: "ที่ปรึกษา",
        team3: "รักษาการแทนผู้อำนวยการ",
        related: "หน่วยงานที่เกี่ยวข้อง",
        related1: "ฝ่ายวิจัยและการถ่ายทอดเทคโนโลยี คณะแพทยศาสตร์มหาวิทยาลัยขอนแก่น และ...",
        menuWork: "งานของเรา",
        videoWork: "คิดได้คิดดี-แอปพลิเคชัน คัดกรองภาวะออทิซึมสเปกตรัมในเด็ก",
        research: "งานด้านการวิจัย",
        active: "งานวิจัยที่อยู่ระหว่างการดำเนินการ",
        service: "งานด้านการบริการ",
        menuNews: "ข่าวสารและประชาสัมพันธ์",
        menuDonate: "บริจาค",
        rsia: "สถาบันวิจัยและบริการด้านออทิซึม",
        rsiakku: "Research And Service Institute For Autism Khon Kaen University",
        button: "อ่านเพิ่มเติม",
        footerInfo:
            "สถาบันวิจัยและบริการด้านออทิซึม สำนักงานอธิการบดี มหาวิทยาลัยขอนแก่น ได้รับอนุมัติขึ้นตามข้อสั่งการของนายกรัฐมนตรี พลเอกประยุทธ์ จันทร์โอชา ณ วันที่ 21 มิถุนายน 2560 เป็นหน่วยงานสนับสนุนภารกิจและยุทธศาสตร์ของมหาวิทยาลัยขอนแก่น ในการดูแลบุคคลออทิซึมให้สามารถช่วยเหลือตนเองและอยู่ร่วมกับสังคมอย่างมีความสุข ตามยุทธศาสตร์ชาติ 20 ปี ยุทธศาสตร์ที่ 4 การสร้างโอกาสและความเสมอภาคทางสังคม และแผนพัฒนาเศรษฐกิจและสังคมแห่งชาติ ฉบับที่ 12 พ.ศ.2560-2564",
        contact: "ติดต่อเรา",
        contact1: "Contact",
        address:
            "สถาบันวิจัยและบริการด้านออทิซึม อาคารวิจัยเพื่อพัฒนาสังคม (RSDI) มหาวิทยาลัยขอนแก่น เลขที่ 123 หมู่ที่ 16 ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น 40002",
        phone: "043-009-700 เบอร์ภายใน 50491, 50492",
        licences: "© 2021 สถาบันวิจัยและบริการด้านออทิซึม RSIA",
        researchwork1:
            "การพัฒนานวัตกรรมสำหรับการช่วยเหลือบุคคลที่มีภาวะออทิซึมสเปกตรัมที่สอดคล้องกับบริบททางสังคมไทยตาม4 กลไกหลักของออทิซึมสเปกตรัมโรดแมป มีวัตถุประสงค์ตั้งแต่การคิดค้นหาวิธีการคัดกรองเด็กเล็กที่มีอาการที่เสี่ยงต่อการเป็นโรคออทิซึมสเปกตรัมจนถึงค้นหาวิธีการดูแลผู้ป่วยที่เป็นโรคออทิซึมสเปกตรัมตั้งแต่อายุน้อยจนถึงวัยรุ่น ผู้ป่วยที่เป็นเด็กอายุน้อยแผนการวิจัยก็คิดค้นวิธีการให้การศึกษาเพื่อให้การศึกษาก้าวหน้าผู้ป่วยที่เป็นวัยรุ่นแผนการวิจัยก็คิดค้นหาวิธีส่งเสริมให้ครอบครัวมีวิธีการส่งเสริมให้วัยรุ่นมีทักษะทางสังคมดีขึ้นแผนการวิจัยจึงประกอบด้วยโครงการวิจัยย่อย 4 โครงการดังนี้",
    },
};
